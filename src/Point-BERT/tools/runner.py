...
def compute_loss(loss_1, loss_2, config, niter, train_writer):
    ...
    if train_writer is not None:
        train_writer.log({'Loss/Batch/KLD_Weight': kld_weight}, step=niter)
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    if train_writer is not None:  
        train_writer.finish()
    if val_writer is not None and val_writer != train_writer:
        val_writer.finish()
...
def validate(base_model, test_dataloader, epoch, ChamferDisL1, ChamferDisL2, val_writer, args, config, logger = None):
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            vis_list = [0, 1, 1600, 1800, 2400, 3400]
            if val_writer is not None and idx in vis_list: #% 200 == 0:
                input_pc = points.squeeze().detach().cpu().numpy()
                # img_input_pc = misc.get_ptcloud_img(input_pc)
                # val_writer.log({'Model%02d/Input' % idx: wandb.Image(input_pc)}, step=epoch)
                # misc.save_ptcloud_as_stl(input_pc, f'./vis/Model{idx:02d}_Input.stl')
                misc.save_ptcloud(input_pc, f'./vis/Model{idx:02d}_Input.ply')

                sparse = coarse_points.squeeze().cpu().numpy()
                # sparse_img = misc.get_ptcloud_img(sparse)
                # val_writer.log({'Model%02d/Sparse' % idx: wandb.Image(sparse_img)}, step=epoch)
                # misc.save_ptcloud_as_stl(sparse, f'./vis/Model{idx:02d}_Sparse.stl')
                misc.save_ptcloud(sparse, f'./vis/Model{idx:02d}_Sparse.ply')

                dense = dense_points.squeeze().cpu().numpy()
                # dense_img = misc.get_ptcloud_img(dense)
                # val_writer.log({'Model%02d/Dense' % idx: wandb.Image(dense_img)}, step=epoch)
                # misc.save_ptcloud_as_stl(dense, f'./vis/Model{idx:02d}_Dense.stl')
                misc.save_ptcloud(dense, f'./vis/Model{idx:02d}_Dense.ply')
        ...
        wandb.log({
            'Val/Epoch/Sparse_L1': test_losses.avg(0),
            'Val/Epoch/Sparse_L2': test_losses.avg(1),
            'Val/Epoch/Dense_L1': test_losses.avg(2),
            'Val/Epoch/Dense_L2': test_losses.avg(3),
            'Val/Epoch/Chamfer_L1': test_metrics.avg(0),
            'Val/Epoch/Chamfer_L2': test_metrics.avg(1),
            'Val/Epoch/EMD': test_metrics.avg(2),
        }, step=epoch)
        
        if args.distributed:
            torch.cuda.synchronize()

    # Add testing results to wandb
    if val_writer is not None:
        metrics_dict = {
            'Loss/Epoch/Sparse': test_losses.avg(0),
            'Loss/Epoch/Dense': test_losses.avg(2)
        }
        for i, metric in enumerate(test_metrics.items):
            metrics_dict['Metric/%s' % metric] = test_metrics.avg(i)
        val_writer.log(metrics_dict, step=epoch)

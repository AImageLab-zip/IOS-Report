...
from segmentation.models.pointnet2_utils import farthest_point_sample, index_points
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
            ...
            fps_idx = furthest_point_sample(points, point_all)  # (B, npoint)
            fps_idx = fps_idx[:, np.random.choice(point_all, npoints, False)]
            points = index_points(points, fps_idx)  # (B, N, 3)
            ...
            if train_writer is not None:
                train_writer.log({
                    'Loss/Batch/Loss': loss.item(),
                    'Loss/Batch/TrainAcc': acc.item(),
...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger = None):
    ...
    if val_writer is not None:
        val_writer.log({'Metric/ACC': acc}, step=epoch)
...
def validate_vote(base_model, test_dataloader, epoch, val_writer, args, config, logger = None, times = 10):
    ...
    # Add testing results to wandb
    if val_writer is not None:
        val_writer.log({'Metric/ACC_vote': acc}, step=epoch)

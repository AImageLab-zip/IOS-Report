...
def save_ptcloud(ptcloud, filename):
    ...
    point_cloud = np.array(ptcloud)
    if point_cloud.ndim == 2 and point_cloud.shape[1] > 3:
        point_cloud = point_cloud[:, :3]
    vertices = np.array([tuple(point) for point in point_cloud],
                        dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4')])
    ply = PlyData([PlyElement.describe(vertices, 'vertex')], text=True)
    ply.write(filename)
    
def save_ptcloud_as_stl(ptcloud, filename):
    from stl import mesh

    # Create a new mesh
    point_cloud = np.array(ptcloud)
    
    # Ensure we only use the first 3 coordinates (x, y, z)
    if point_cloud.ndim == 2 and point_cloud.shape[1] > 3:
        point_cloud = point_cloud[:, :3]
    
    num_points = point_cloud.shape[0]
    # Create the mesh
    point_mesh = mesh.Mesh(np.zeros(num_points, dtype=mesh.Mesh.dtype))
    for i in range(num_points):
        point_mesh.vectors[i] = np.array([point_cloud[i], point_cloud[i], point_cloud[i]]) 
        

    # Save the mesh to file
    point_mesh.save(filename)
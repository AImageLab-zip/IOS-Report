...
def _load_patient_data_worker(args: Tuple) -> Tuple[int, Dict]:
    ...
    Args:
        args: Tuple of (idx, root_dir, patient_id, n_points, load_meshes, load_images, image_size)
        
    Returns:
        Tuple of (idx, data_dict)
    """
    idx, root_dir, patient_id, n_points, load_meshes, load_images, image_size = args
...
class FerraraDumpDataset(Dataset):
    ...
    def _load_images(self, photos_dir: Path) -> List[Image.Image]:
        ...
        for img_name in ["left", "center", "right", "upper", "lower"]:
